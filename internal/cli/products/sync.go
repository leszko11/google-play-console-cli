...
func newSyncCommand(deps Deps) *ffcli.Command {
	...
	return &ffcli.Command{
		...
		Exec: func(ctx context.Context, _ []string) error {
			var err error
			opts.Dir, err = shared.ResolveProjectPath(opts.Dir, func(cfg config.ProjectConfig) string { return cfg.ProductsDir })
			if err != nil {
				return err
			}
...
func runSync(parentCtx, requestCtx context.Context, client SyncClient, out io.Writer, opts syncOptions, products []*androidpublisher.OneTimeProduct) error {
	result := syncResult{
...
func RunSync(parentCtx, requestCtx context.Context, client SyncClient, out io.Writer, opts SyncOptions) error {
	products, err := readSyncProductsDir(opts.Dir)
	if err != nil {
		return err
	}
	return runSync(parentCtx, requestCtx, client, out, opts, products)
}
